#!/usr/bin/env python3
"""Prove wheel/sdist identity: ordered-fold 1.0.0.dev0, core floor, BSD-2-Clause."""
...
EXPECTED_NAME = 'django-trusts-ordered-fold'
...
def _check_metadata(meta_text: str, origin: str) -> None:
    ...
    if 'pip install django-trusts-ordered-fold' not in description:
        raise SystemExit('%s long description is not the user README' % origin)
    if 'baton' in description or 'kernel_config' in description:
...
def _check_wheel(wheel: Path) -> None:
    with zipfile.ZipFile(wheel) as zf:
        ...
        if any(name.startswith('trusts/') for name in names):
            raise SystemExit('wheel must not ship trusts.*: %s' % wheel.name)
...
def _check_sdist(sdist: Path) -> None:
    with tarfile.open(sdist, 'r:gz') as tf:
        names = tf.getnames()
        prefix = 'django_trusts_ordered_fold-1.0.0.dev0'
...
def main() -> int:
    dist = _dist()
    wheels = sorted(dist.glob('django_trusts_ordered_fold-*.whl'))
    sdists = sorted(dist.glob('django_trusts_ordered_fold-*.tar.gz'))
    wheel = _require_dist(wheels, 'wheel')
    sdist = _require_dist(sdists, 'sdist')
    _check_wheel(wheel)
    _check_sdist(sdist)
    print('package metadata ok')
    print('ordered-fold', EXPECTED_NAME + '==' + EXPECTED_VERSION)
